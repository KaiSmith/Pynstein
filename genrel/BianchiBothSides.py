...
def scale_factors(Ha, Hb, Hc):
    print(sorted(Ha.keys()))
    dadt = make_dSdt(Ha)
    dbdt = make_dSdt(Hb)
    dcdt = make_dSdt(Hc)
    return RK4(dadt, a0, sorted(Ha.keys())[0], sorted(Ha.keys())[-1], step), RK4(dbdt, b0, start, stop, step), RK4(dcdt, c0, start, stop, step)

def plot_hubble_parameters(show_plot = True):
...
if __name__ == '__main__':
    ...
    Hab, Hbb, Hcb = plot_hubble_parameters(False)
    ...
    Haf, Hbf, Hcf = plot_hubble_parameters())

    t = np.linspace(-.4, .45, abs((.85)/.01)+1)
